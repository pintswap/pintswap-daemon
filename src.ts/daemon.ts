...
export const logger = createLogger('pintswap-daemon');
...
export const PINTSWAP_DIRECTORY = path.join(process.env.HOME, '.pintswap-daemon');

export const PINTSWAP_PEERID_FILEPATH = path.join(PINTSWAP_DIRECTORY, 'peer-id.json');
...
export async function loadOrCreatePeerId(){
  ...
  await fs.writeFile(PINTSWAP_PEERID_FILEPATH, JSON.stringify(peerId.toJSON(), null, 2));
  return peerId;
}
...
export async function run() {
  ...
  const pintswap = new Pintswap({ signer: wallet, peerId });
  ...
  rpc.post('/offers', (req, res) => { 
    const offers = [ ...pintswap.offers ].map(([ k, v ]) => ({ ...v, id: k, link: 'https://pintswap.eth.limo/#/' + peerId.toB58String() + '/' + k }));
    res.json({
      status: 'OK',
      result: offers
    });
