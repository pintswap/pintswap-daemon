...
export function orderTokens(offer: any) {
  ...
  if (mapped.givesToken === USDC.address) {
    ...
    Number(mapped.givesToken.toLowerCase()) <
...
export async function fromFormatted(trade: any, provider: any) {
  const [givesToken, getsToken] = [trade.givesToken, trade.getsToken].map((v) =>
    toAddress(v),
  );
  return {
    givesToken,
    ...
    givesAmount: ethers.toBeHex(
      ethers.parseUnits(
        trade.givesAmount,
        await getDecimals(givesToken, provider),
      ),
    ),
    getsAmount: ethers.toBeHex(
      ethers.parseUnits(
        trade.getsAmount,
        await getDecimals(getsToken, provider),
      ),
    ),
  };
}

export async function fromLimitOrder({ price, amount, type, pair }, provider) {
  const [trade, base] = pair.split("/").map(toAddress);
  const [tradeDecimals, baseDecimals] = await Promise.all(
    [trade, base].map(async (v) => await getDecimals(v, provider)),
  );
  if (type === "bid") {
    return {
      getsToken: trade,
      givesToken: base,
      getsAmount: ethers.toBeHex(
        BigInt(Math.floor(Number(amount) * 10 ** tradeDecimals)),
      ),
      givesAmount: ethers.toBeHex(
        BigInt(
          Math.floor(
            Number(price) * Number(amount) * Number(10 ** baseDecimals),
          ),
        ),
      ),
    };
