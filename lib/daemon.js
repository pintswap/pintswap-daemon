...
exports.PintswapDaemon = exports.PINTSWAP_DATA_FILEPATH = exports.expandOffer = exports.expandValues = exports.PINTSWAP_PEERID_FILEPATH = exports.PINTSWAP_DIRECTORY = exports.providerFromEnv = exports.walletFromEnv = exports.logger = exports.toProvider = exports.signBundle = void 0;
...
function mapBigIntToHexRecursive(o) {
    if (Array.isArray(o))
        return o.map((v) => mapBigIntToHexRecursive(v));
    if (o === null)
        return o;
    if (typeof o === 'object')
        return (0, lodash_1.mapValues)(o, (v) => mapBigIntToHexRecursive(v));
    if (typeof o === 'bigint')
...
function providerFromEnv(chainId) {
    const _chainId = Number(chainId || process.env.PINTSWAP_DAEMON_CHAINID || 1);
    return (0, sdk_1.providerFromChainId)(_chainId);
}
...
class PintswapDaemon {
    ...
    sendBundle(packed, blockNumber) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield (0, utils_1.sendBundle)(this.pintswap.logger, this.flashbots, packed, blockNumber + 1);
        });
    }
    loadOrCreatePeerId() {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    createHandlers() {
        let publisher;
        const chainId = (req, res) => {
            (() => __awaiter(this, void 0, void 0, function* () {
                try {
                    let { id } = req.body;
                    this.wallet = walletFromEnv().connect(providerFromEnv(Number(id)));
                    this.pintswap.signer = walletFromEnv();
                    res.json({
                        status: "OK",
            ...
            try {
                ...
                providerProxy.estimateGas = function (...args) {
                    return __awaiter(this, void 0, void 0, function* () {
                        const [txParams] = args;
                        if (!txParams.to) {
                            const gasLimitEstimate = yield estimateGasBound(...args);
                            if (BigInt(gasLimitEstimate) > GASLIMIT_CAP)
                                return GASLIMIT_CAP;
                            else
                ...
                if (broadcast) {
                    ...
                    const packed = yield (0, repack_1.pack)(this.pintswap.signer, txs.map((v) => v.transaction));
                    exports.logger.info(mapBigIntToHexRecursive(yield (0, utils_1.callBundle)(this.pintswap.logger, this.flashbots, packed, blockNumber + 1)));
                    const bundleResult = (yield this.sendBundle(packed, blockNumber + 1));
                    exports.logger.info(bundleResult);
