...
function orderTokens(offer) {
    ...
    else if (Number(mapped.givesToken.toLowerCase()) <
        Number(mapped.getsToken.toLowerCase())) {
        return givesBase(mapped);
    }
    else
        return givesTrade(mapped);
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (type === "bid") {
            return {
                getsToken: trade,
                givesToken: base,
                getsAmount: ethers_1.ethers.toBeHex(BigInt(Math.floor(Number(amount) * Math.pow(10, tradeDecimals)))),
                givesAmount: ethers_1.ethers.toBeHex(BigInt(Math.floor(Number(price) * Number(amount) * Number(Math.pow(10, baseDecimals))))),
            };
        }
        if (type === "ask")
            return {
                getsToken: base,
                givesToken: trade,
                givesAmount: ethers_1.ethers.toBeHex(BigInt(Math.floor(Number(amount) * Math.pow(10, tradeDecimals)))),
                getsAmount: ethers_1.ethers.toBeHex(BigInt(Math.floor(Number(price) * Number(amount) * Number(Math.pow(10, baseDecimals))))),
            };
    });
}
